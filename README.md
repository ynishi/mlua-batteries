...
Every function raises on failure; the declarations carry that in a header comment rather than `, string` return types, so wrap a call in `pcall` for a result. `json.decode` / `json.read_file` are declared twice — `function(string): any` and `function<T>(string): T` — so `pcall(json.decode, s)` and a bare call type-check, and an annotated `local r: R = json.decode(s)` gets its type without a cast (a generic alone does not resolve through `pcall`). `json.null` is the explicit-null sentinel (`T | json.Null` in a record). A function that may return nil (`path.parent`, `env.get`, `regex.find`, …) is declared as the plain type with a comment: Teal has no non-nil types, so this is not something the checker or htl's lints can see. The declarations are checked against the modules by `tests/dts_drift.rs` (field names both ways, plus `htl check` over consumer-shaped calls when htl is installed).
