...
Every function raises on failure; the declarations carry that in a header comment rather than `, string` return types, so wrap a call in `pcall` for a result. `json.decode` / `json.read_file` are declared twice — `function(string): any` and `function<T>(string): T` — so `pcall(json.decode, s)` and a bare call type-check, and an annotated `local r: R = json.decode(s)` gets its type without a cast (a generic alone does not resolve through `pcall`). `json.null` is the explicit-null sentinel (`T | json.Null` in a record). Optional trailing arguments are declared `name?: T` (`pretty.dump(v)`, `string.pad_start(s, n)`, `log.info(msg)` check without the argument). Records a caller builds — `http.Request`, `llm.Request` / `Message` / `ContentPart`, `proc.Stage` / `FileRef`, `argparse.Positional` — carry htl's `---@struct` / `---@optional` markers, so a literal that leaves a required field out is a `struct-fields` lint; records the modules return, and option tables whose fields are all optional, carry none. A function that may return nil (`path.parent`, `env.get`, `regex.find`, …) is declared as the plain type with a comment and a trailing `---@nilable`: Teal has no non-nil types, the marker is a comment to today's htl, and it is reserved for the lint htl plans. The declarations are checked against the modules by `tests/dts_drift.rs` (field names both ways, plus `htl check` over consumer-shaped calls when htl is installed).
